...
bool VCU_CAN_Start(void) {
  ...
  filter_config.FilterID1 = 0x100; /* Start ID */
  filter_config.FilterID2 = 0x10F; /* End ID */
  ...
  filter_config.FilterType = FDCAN_FILTER_RANGE;
  filter_config.FilterID1 = 0x900 + VESC_CAN_ID; /* VESC status base */
  ...
  if (HAL_FDCAN_ConfigGlobalFilter(hfdcan_ptr, FDCAN_REJECT, FDCAN_REJECT,
                                   FDCAN_FILTER_REMOTE,
  ...
  /* Activate RX FIFO 0 interrupt */
  if (HAL_FDCAN_ActivateNotification(hfdcan_ptr, FDCAN_IT_RX_FIFO0_NEW_MESSAGE,
                                     0) != HAL_OK) {
    return false;
  }
